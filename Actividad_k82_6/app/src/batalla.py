...
# Modifica la función `disparar` existente para registrar las métricas
def disparar(x, y, matriz) -> bool:
    if es_mar(x, y, matriz):
        matriz[y][x] = DISPARO_FALLADO
        requests.post("http://app:5000/send_stat", json={"acierto": False, "method": "POST", "endpoint": "/send_stat", "status_code": 200})
        return False
        ...
        requests.post("http://app:5000/send_stat", json={"acierto": True, "method": "POST", "endpoint": "/send_stat", "status_code": 200})
        return True

# Resto del código para la lógica del juego
